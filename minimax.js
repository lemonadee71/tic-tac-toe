...
const minimax = (board, player, enemy, depth, winningPattern) => {
  ...
  for (let i = 0; i < board.length; i++) {   
    ...
    let choice = Math.floor(Math.random() * (currentBoard.length - 1))
    currentBoard.splice(choice, 1)

    console.log(choice)
    if (depth % 2 === 0)
      newPlayer.playerMoves.push(currentBoard[choice])
    else 
      newEnemy.playerMoves.push(currentBoard[choice])
